...
var IndianOfficer, trex_running, trex_collided;
...
function preload(){
  ...
  gameOverSound = loadSound("game-over.m4a");
  JumpSound = loadSound("JumpSound.wav");

  IndianOfficer_running =   loadImage("trexImg.png");
  ...
  enemytankImg=loadImage("enemyTank.png");
}

function setup() {
  createCanvas(1200, 500);
  
  IndianOfficer = createSprite(40,40,0,0);
  
  IndianOfficer.addImage("running", IndianOfficer_running);
  IndianOfficer.addAnimation("collided", trex_collided);
  IndianOfficer.scale = 0.75;
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if(keyDown("space") && IndianOfficer.y >= 159) {
      IndianOfficer.velocityY = -12;
      JumpSound.play();
    }
    
    IndianOfficer.velocityY = IndianOfficer.velocityY + 0.8
  
    if (ground.x < 550){
      ground.x = ground.width/2;
    }
  
    IndianOfficer.collide(invisibleGround);
    spawnClouds();
    spawnObstacles();
  
    if(obstaclesGroup.isTouching(IndianOfficer)){
        gameState = END;
    ...
    ground.velocityX = 0;
    IndianOfficer.velocityY = 0;
    obstaclesGroup.setVelocityXEach(0);
    cloudsGroup.setVelocityXEach(0);
    
    //change the trex animation
    IndianOfficer.changeAnimation("collided",trex_collided);
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
   ...
   // cloud.depth = trex.depth;
    IndianOfficer.depth = IndianOfficer.depth + 1;
    cloud.visible=false;
...
function reset(){
  ...
  IndianOfficer.changeAnimation("running",trex_running);
